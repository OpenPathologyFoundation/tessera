...
| DCR-037 | Design Change Record — The Review Package Names Its Version | Change Control | Draft |
| DCR-038 | Design Change Record — Pre-Validation Corrections | Change Control | Draft |
| DCR-039 | Design Change Record — The Majors Behind the Blockers | Change Control | Draft |
...
| AO | 2026-08-08 | QMS | v2.22.3 (DCR-039): the majors standing behind DCR-038's blockers. SOP-001 described four things that do not exist — a fixed key assignment (contradicting its own §4.1 and the editor, in the **limitations list**, where a false limit is believed), a case-number-change dialog, a v2.5 profile stamp on v2.6 tables, and a revision history ending at Rev A under a v2.0 header. README promised a blocking below-target warning; completion has never been blocked. Two profile defects printed into clinical output: `analyzer-5` stated a 200-cell rationale on its 100-cell reports, and `body-fluid` carried the marrow note telling operators not to count macrophages and metastatic tumour cells — on the panel with `malignant` and `mono_macro` keys for exactly those. **And the first code defect this log has recorded since the drift work began:** a key captured with Shift validated cleanly and produced a category that could be decremented but never incremented, because validation checked uniqueness and mapping but never typeability. `isCountableKey` rejects it, in the engine and at the editor's capture. UD-100, UD-101; incidents 37–43. |
| AN | 2026-08-08 | QMS | v2.22.2 (DCR-038): a pre-validation audit of every surface a validating pathologist reads found seven blockers, **all of them in documentation and none in the software**. SOP-001's miscount-correction example named A=blast and V=lymph when A is monocytes and V is myelocytes — a wrong keystroke in the procedure written to prevent wrong keystrokes. VV-001's scenario V6 instructed keys R, P and L, of which P and L are unmapped, so the validation script could not be executed against the profile it validates. The monthly QC entered categories that do not exist and cited a test that does not. The data-handling attestation denied localStorage while autosave writes the accession number to it. Two sections described a disabled Start button that was never built — so the procedural control it stood in for was never written down. The offline help page listed seven withdrawn profiles. The distribution is the finding: the guards built so far check generated figures and profile names, and nothing checked the body of a procedure. UD-097/098/099 now do. |
| AM | 2026-08-07 | QMS | v2.22.1 (DCR-037): the clinical review package was about to be sent and could not be acted on. It named no version, so a signature would have attached to no particular state of the file; it gave no route from reading the brief to opening the tool; and — once the hosted site at mdc.openpathology.org became the route — the application displayed only the PROFILE version, so a reviewer could have taken `ndc-14 v2.6` for the version under review. The method statement now states `Application: WBC ΔΣ v2.22.1` beside it, QC-016 holds that equal to package.json, DHF-001 and the cache key, and every exported report now names the build that produced it. Documentation-only otherwise; v2.22.0 stands rather than being re-tagged, because moving a published tag would make the pin unverifiable. |
