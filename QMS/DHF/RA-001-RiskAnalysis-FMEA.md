...
| HA-107 | A cell type present on the slide has **no category at all** in the loaded profile | The aggregated 10-type profiles have no plasma-cell or mast-cell category, and one has no promyelocyte category. An operator meeting such a cell has no key for it and may count it into the nearest category — plasma cells as lymphocytes, mast cells as basophils — which overstates that category and depresses every other percentage, because the denominator grows by a cell that does not belong to it. Unlike HA-090 the cell is a legitimate member of the differential; it is the profile that is incomplete | 3 | A profile whose categories are an aggregated subset of the ICSH nucleated differential count is loaded for a marrow containing a category it does not carry | 3 | **Partial** — the count remains internally consistent and the report names the profile, so a reader who checks the profile can see which categories existed. Nothing in the count itself reveals the substitution | 4 | **36** | **Medium** | Category guidance on the lymphocyte, basophil and myelocyte rows of the affected profiles states that the cell has no category here, instructs the operator to record it in the morphology comment instead, and names the profile that does carry it. The profile description says the same before the profile is loaded. VV-PRE-034 fails the build if a profile omits an ICSH category without carrying that guidance | 3x3x2 = **18** (Medium). **Residual risk accepted with documentation control.** The guidance cannot prevent an operator who ignores it, and a smaller panel is a legitimate choice a laboratory makes; what is controlled is that the omission is stated where the substitution would be made rather than left to be discovered. See DCR-035 | |
| HA-108 | The operator comes to rely on audio, and it becomes unavailable | An AudioContext may be refused (no user gesture yet, a locked-down browser, a machine with no output device). If tonal feedback has become the operator's wrong-key check, losing it silently removes a control they believe they have | 2 | Browser policy, hardware, or the operator muting the machine | 3 | **Effective** — the visual flash is primary and unchanged in every audio mode, so nothing the operator relies on for correctness is carried by sound alone. TC-B098 asserts the count and the flash are identical in off, click and tones | 2 | **12** | **Low** | Audio is supplementary by design (SYS-258, URS-108). The application degrades silently rather than warning, because a warning about a supplementary channel would interrupt a count for no clinical reason | 2x3x2 = **12** (Low). **Accepted.** See DCR-036 | |
| HA-109 | Two adjacent categories are confused by ear | A tone identifies a category only if it is distinguishable from its neighbours. Categories crowded in pitch would let a wrong key pass as right — the opposite of the control's purpose | 3 | A scale with small intervals, or a profile with many categories | 2 | **Effective** — the minor pentatonic contains no semitone intervals, so adjacent categories are at least two semitones apart by construction at every profile size; per-press detune is bounded at ±10 cents, an order of magnitude inside that gap. VV-TON-002 asserts the spacing for n = 1..24, VV-TON-012 the jitter bound | 2 | **12** | **Low** | The claim made to the operator is discrimination, not identification: they are not asked to name a pitch, only to notice a change. URS-108 states it that way | 3x2x2 = **12** (Low). **Accepted.** See DCR-036 | |
| HA-110 | The ambient pitch texture biases cell classification | Because pitch follows category order, a count acquires an audible texture: a marrow rich in the earliest categories sounds darker as it proceeds. That drift is perceptible without being counted — and the operator's task is **classifying ambiguous cells**. An involuntary cue correlating with the emerging picture is a plausible anchoring mechanism: a marrow that starts *sounding* blast-rich at cell 80 may bias the borderline cells at cell 200. Unlike the wrong-key control, which is a detection aid with a clear error model, this influences judgement and has none | 3 | Pitch derived from category order, which is what makes the wrong-key control work; the two cannot be separated | 3 | **None** — nothing in the count reveals whether it happened, and the operator cannot report a bias they did not notice | 5 | **45** | **Medium** | Tones are **not the shipped default** and must be selected. The visual flash and the running percentages remain primary. The texture is deliberately **not described in operator-facing documentation**: naming it is what would prime the effect, and the wrong-key benefit does not require the operator to know about it. Note also that the texture is a weaker signal than it appears — in the shipped orderings the darkest tone is erythroid, so an erythroid-hyperplastic marrow sounds darker than a blast-rich one, and the cue conflates states a clinician would separate | 3x3x3 = **27** (Medium). **Residual risk accepted pending clinical review**, which DCR-036 §7 invites specifically on this hazard. If a reviewer judges the bias material, the mitigation is to break the correspondence between pitch order and maturation order — the mapping is derived, so that is a one-line change | |
...
| HA-093 | The M:E ratio is displayed to a precision the count does not support | A ratio of two counted proportions carries the sampling error of both and is substantially less precise than either. Displaying "2.1:1" invites comparison between successive marrow examinations at a resolution the counts cannot sustain, potentially reading a change in disease where only sampling noise exists | 2 | The ratio is rendered at the precision the configuration specifies, with nothing stated about its uncertainty | 3 | **None** | 3 | **18** | **Medium** | **Closed 2026-08-06 (DCR-026).** The ratio now carries a confidence interval, displayed beside it and governed by the same profile setting as the percentage intervals. It is exact rather than approximate: conditioning on the cells that fall in either group leaves a single binomial, so M:E is the odds of that proportion and the interval is the odds transform of the Wilson interval already adopted under DCR-007. Fieller's theorem was rejected because it degenerates with no erythroid cells — the case most needing a bound — and a bootstrap because it would give a different answer on each run of the same count. The prose advisory is retained alongside it. VV-ME-010..016, VV-SYS-201..203. | 3x1x1 = **3** (Low). The same 2.3:1 now reads 1.8–3.0 at 300 cells and 1.1–5.0 at 30, so the reader sees the imprecision rather than being told about it. Regression detection confirmed: removing the display fails VV-SYS-201/202; breaking the odds transform fails VV-ME-010. |
...
| HA-098 | A clinical advisory is displayed but cannot be read | The near-threshold advisory, the sub-target note, the abnormal-row flag and the peripheral blood per-100 value all used an amber palette chosen for a dark background. In the light theme they rendered at a contrast ratio of **1.28:1** against their panel — effectively invisible. The system recorded that it had warned; the operator saw nothing. Worse than issuing no advisory, because the count carries a note in its record that the operator never read | 4 | The light theme defined overrides for every slate tone but none for amber or red; every test asserted on advisory *text content*, which was present and correct | 3 | **None** — the element is in the DOM, populated, and reported visible by any content-based check | 5 | **60** | **High** | Light-theme overrides added for the full amber and red scales, chosen to clear WCAG AA. **Scope was then found to be wider than the reported defect**: a full-surface sweep (VV-SYS-162..168, every text node on every page in both themes) reported 330 further failures — the help page carried no theme overrides at all, the documentation pages and the editor each maintained a partial copy, and the two primary action buttons rendered white on amber/emerald at 3.19:1 and 3.77:1 **in both themes**, which no light-theme-only check could have detected. All five per-page blocks were consolidated into `web/styles/theme.css`, every muted tone recalibrated against the lightest surface it is used on, and the theme attribute moved to `<html>` and applied in `<head>` so it takes effect before first paint (previously the page painted dark, then transitioned, putting text transiently below AA). | 4x1x1 = **4** (Low). Regression detection confirmed by reverting each fix in turn: the accent tone, the button backgrounds and the paint-order change are each detected by a named test. |
| HA-099 | A configuration tool silently discards the clinical policy of the profile it is editing | The Configuration Editor rebuilt each profile from its own form fields, so every field it has no control for was destroyed on save: `denominatorExcludes`, `per100Reporting`, `thresholds`, `confidenceIntervals`, `rounding`, `precision`, `categoryNotes`, `targetCountBasis` and `provenance`, with `formulas` overwritten by `{}` — deleting the M:E ratio. Saving the shipped profile untouched returned a profile that counts differently, while reporting "Profile saved and made active". For peripheral blood this is a direct route back into HA-092 | 4 | The editor models layout, keys, templates and a few settings; the counting policy is invisible to it, and rebuilding from a literal made omission indistinguishable from deletion | 3 | **None** — the result is a schema-valid profile, so `validateConfig` accepts it. It simply counts differently | 5 | **60** | **High** | The source profile is retained on load and merged on save; the editor overrides only what it actually edits. VV-SYS-063 loads the shipped profile, saves it untouched and asserts every top-level and per-specimen field deep-equal. | 4x1x1 = **4** (Low). Regression detection confirmed: with the merge removed, VV-SYS-063 fails. |
| HA-100 | A saved configuration is not the configuration in use | The editor wrote a hard-coded `version: '2.0'`. Because the built-in profile shares the `profileId` at a higher version, `isCacheSuperseded` discarded the saved profile on the next load. The editor reported "saved and made active"; the counter announced a routine profile update and used the built-in. A laboratory could operate believing its local policy — target counts, categories, denominators — was active when it was not | 4 | The supersede mechanism exists to deliver corrected profiles to installed browsers; nothing distinguished a stale cache from a deliberate local edit | 2 | **Low** — a modal appears, but it describes a vendor update rather than the loss of the operator's own change, and contradicts the editor's success message | 4 | **32** | **Medium** | The saved version is carried forward from the source profile with its last component incremented, so an edit is a newer revision of its parent and is not superseded by it. VV-SYS-064 edits a built-in profile, keeps its `profileId`, and asserts the counter honours the change. | 4x1x1 = **4** (Low). Regression detection confirmed: with the version hard-coded again, VV-SYS-064 fails. |
| HA-101 | Input is rejected without the operator being told why | The configuration editor asked for a specimen type identifier and a derived-figure name with the browser's `prompt()`. A prompt cannot state a rule, cannot show which identifiers are already taken, and cannot refuse input except by discarding it and reopening. "Body Fluid" was accepted as an identifier and produced a profile that behaved oddly later; a duplicate identifier silently shadowed an existing specimen | 2 | Native dialogs were used because they are one line of code; nothing in the design file required the product to ask its own questions | 4 | **Low** — the operator sees a result, but not the reason, and may repeat the same input | 3 | **24** | **Low** | SYS-244/245: one dialog widget for the whole product. Rules are stated as a hint, both values are asked for at once, validation runs on submission and the reason appears beneath the offending field with what was typed intact. VV-SYS-170..172; suite 04 bans the native calls from shipped code. | 2x1x1 = **2** (Low). Regression detection confirmed: restoring the prompts fails VV-SYS-170, 172 and 173. |
| HA-102 | A dialog keystroke reaches the count, or dismisses a consequential choice | Two distinct failures of modality. The counter binds counting keys to the document and skipped only keystrokes aimed at form controls, so a counting key pressed while the Reset confirmation was open — where focus is on a button — was still tallied. Separately, a stray Escape on the interrupted-count prompt would have chosen Discard, throwing away a recovered count | 4 | The dialog was styled but not modal in behaviour: no focus containment, no keyboard ownership, and no distinction between a dismissible dialog and a forced choice | 3 | **None** — the tally moves silently; the discard is indistinguishable from a deliberate one | 3 | **36** | **Medium** | SYS-246/247: the counter ignores counting keys while a dialog is open; focus moves into the dialog, is confined to it and is restored on close; Escape cancels except where both branches are consequential, and the recovery prompt is opened non-dismissible. VV-SYS-173..176. | 4x1x1 = **4** (Low). Regression detection confirmed by removing the keyboard guard and by making the recovery prompt dismissible; each is caught by a named test. |
| HA-103 | A held or bouncing key inflates the count | `onKeyDown` never checked `ev.repeat`. A held, sticky or bouncing key adds cells at the operating system's auto-repeat rate — roughly 30 per second, each with its own confirmation sound — and nothing in the interface distinguishes that from fast deliberate counting. At a 200-500 cell target a two-second stuck key is a plausible and material miscount, in the direction of over-counting whichever category the key maps to | 4 | The flag was known and already used by the theme shortcut; it was simply never applied to the counting path, the one place it changes a clinical number | 4 | **None** — the tally, the sounds and the progress bar are identical to real counting | 5 | **80** | **High** | `ev.repeat` and `ev.isComposing` now return early in `onKeyDown`. VV-SYS-181 dispatches one keydown followed by forty repeats and asserts the count is 1; VV-SYS-182 covers input-method composition. | 4x1x1 = **4** (Low). Regression detection confirmed: with the guard removed the test reports 41. |
| HA-104 | A miscount cannot be corrected, silently | Undo is Shift+key, and the handler resolved the mapping from the CHARACTER produced. Shift changes that character for punctuation: on a US layout Shift+"." is ">", Shift+"," is "<", Shift+"/" is "?" and Shift+";" is ":". The shipped `right-hand` preset maps those four keys to blasts, metamyelocytes, basophils and monocytes, so **those four categories could not be un-counted at all** — including blasts, the category most likely to be misidentified and most consequential when wrong. No error, no sound, no visual change; the count simply does not move | 3 | Only the increment path was ever exercised. No test in the project pressed Shift on a mapped key for any shipped preset | 4 | **Low** — the operator may notice the count did not change, or may not | 4 | **48** | **High** | The printed character is tried first, so a laboratory on AZERTY or QWERTZ keeps the key its keyboard is labelled with; if that does not match, the physical key position decides, which Shift does not change. VV-SYS-180 presses and Shift-presses every mapped key in every selectable preset and asserts the count returns to zero. | 3x1x1 = **3** (Low). Regression detection confirmed: with the old resolution the test reports `right-hand.json (bm): Shift+"." did not decrement blasts`. |
| HA-105 | An absolute count is derived from a leucocyte concentration that is not one | Impedance analysers count nucleated red cells as leucocytes, because they resist the lysing reagent. The reported WBC is therefore inflated whenever NRBC circulate, and every absolute count derived from it is overstated by the same factor: at 20 NRBC per 100 WBC, by 20%. The application computed the NRBC per-100 figure whose sole clinical purpose is to feed this correction, printed the formula in its own reference document, and multiplied by the uncorrected value anyway. The absolute neutrophil count drives neutropenia grading and chemotherapy decisions, and a 20% overstatement moves values across the 1.5 and 0.5 x10^9/L boundaries — in exactly the population where nucleated red cells circulate: neonates, severe haemolysis, marrow infiltration | 5 | Excluding NRBC from the denominator and correcting the analyser WBC are two halves of one convention; DCR-006 implemented the first half only, and nothing connected the per-100 figure to the absolute-count path | 3 | **None** — the absolute counts are internally consistent with the percentages and the WBC as entered; nothing on screen indicated a correction was owed | 5 | **75** | **High** | SYS-248/249/250: the correction is applied before any absolute count is derived, and is SHOWN rather than applied silently — the value entered, the arithmetic and the result — because only the operator knows whether the analyser already performed it. A checkbox declares an already-corrected value, which is then used unchanged. The control appears only where the correction applies, so it cannot be used where it is wrong, and marrow profiles are unaffected since ICSH places erythroblasts inside the nucleated differential count. VV-ABS-020..024, VV-SYS-186..190. | 5x1x1 = **5** (Low). Regression detection confirmed: reverting to the uncorrected WBC makes VV-SYS-186 report an ANC of 6.00 where 5.00 is correct. |
| HA-106 | An absolute count is read as zero when none was calculated | Absolute counts derive from an analyser WBC the operator types after the count is finalised. A report template carrying `{{poly_abs}}` is rendered before that value exists, so an unresolved token would print blank or, if defaulted numerically, `0.00`. A blank absolute neutrophil count reads as an omission; a zero reads as a measured absence and is the most alarming value the field can take. Neither is true | 4 | The report is rendered at completion and the WBC is entered afterwards, so the two are never simultaneously available at first render | 2 | **Low** — a reader may notice the absence of a WBC elsewhere in the report, or may not | 3 | **24** | **Low** | SYS-252: absolute-count placeholders resolve to the explicit string "not provided" until an analyser WBC is entered, and the report is then re-rendered. Absolute counts in the report are off by default (SYS-251), so no existing profile acquires the token unasked. VV-SYS-191. | 4x1x1 = **4** (Low). Regression detection confirmed: disabling the re-render leaves "not provided" in place and VV-SYS-191 fails. |
...
| High (50-74) | 9 | HA-003, HA-030, HA-080, HA-092, HA-098, HA-099, HA-104, HA-105; HA-103 at 80 |
...
| L | 2026-08-06 | QMS | v3.0: HA-098 mitigation widened. Verifying the fix by sweeping every text node rather than the reported region found 330 further contrast failures, including two action buttons that failed in **both** themes and a flash of the wrong theme before first paint. Residual rating unchanged at 4; the controls behind it are now product-wide rather than advisory-specific. |
| M | 2026-08-06 | QMS | v3.1: HA-099 and HA-100 added, and two new reachable paths recorded for HA-092. Verifying the configuration user interface after a reviewer could not find `denominatorExcludes` showed the editor destroyed every field it did not model, that its output was then discarded as superseded, and that no shipped preset carried the denominator policy at all. Pre-RPN 60 and 32; residual 4 each. See DCR-012. |
| N | 2026-08-06 | QMS | v3.2: HA-101 and HA-102 added, and HA-098's control widened to interaction states. Replacing the editor's browser prompts with the product's own dialog exposed two modality defects — a counting key pressed over an open dialog was still tallied, and Escape would have discarded a recovered count — and measuring hover states for the first time found every primary button at 3.68:1 under the pointer. Pre-RPN 24 and 36; residual 2 and 4. See DCR-014. |
| O | 2026-08-06 | QMS | v3.3: HA-103 and HA-104 added following independent review. Both are in the INPUT path, which is more likely to produce a wrong clinical number than the arithmetic: key auto-repeat was unguarded, and four categories in a shipped preset could not be un-counted at all. Pre-RPN 80 and 48 — the highest recorded in this file — residual 4 and 3. See DCR-015. |
| P | 2026-08-06 | QMS | v3.4: HA-105 added — an absolute count derived from an analyser WBC that counts nucleated red cells as leucocytes. Pre-RPN 75, severity 5: the absolute neutrophil count drives neutropenia grading and was overstated by 20% at 20 NRBC/100 WBC. The correction is now performed and displayed rather than applied silently. See DCR-016. |
| T | 2026-08-07 | QMS | v3.8 (DCR-036): HA-108, HA-109 and HA-110 added for tonal feedback. HA-110 was not in the endorsed proposal and is the one that matters: pitch follows category order, so a count acquires an audible texture that correlates with the emerging picture, and the operator's task is classifying ambiguous cells. The wrong-key benefit is a detection aid with an error model; the texture influences judgement and has none. Detection scored 5 — nothing in the count reveals whether it happened. Controlled by not defaulting to tones and by keeping the texture out of operator-facing documentation, since describing it is what would prime it. Residual 27, accepted pending clinical review. |
| S | 2026-08-07 | QMS | v3.7 (DCR-035): **HA-107 added.** The aggregated 10-type profiles carry no plasma-cell or mast-cell category, so a marrow containing them offers the operator no key — an ordinary cell of the differential with nowhere to go. HA-090 and HA-092 cover the opposite case, a cell that should not have been counted; neither covered a cell that should have been and could not be. Controlled by category guidance placed where the substitution would be made — on the lymphocyte and basophil rows, not in a general note. Residual 18, accepted. |
| R | 2026-08-06 | QMS | v3.6: **HA-093 closed.** The M:E ratio now carries a confidence interval, closing the gap REF-001 §3.8 recorded and deferred — the imprecision Rümke's 1985 paper actually concerns. Residual 3. See DCR-026. |
| Q | 2026-08-06 | QMS | v3.5: HA-106 added — an absolute count read as zero when none was calculated. Raised while making absolute counts reportable (DCR-016 Rev B): the report renders before the analyser WBC is entered, so the placeholder must state that it was not provided rather than render blank or zero. Pre-RPN 24, residual 4. |
