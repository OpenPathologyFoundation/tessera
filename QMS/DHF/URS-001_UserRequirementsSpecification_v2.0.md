...
| URS-032 | The system SHALL allow the displayed and reported decimal precision to be configured, defaulting to 2 places on screen and 0 in the report. It SHALL state, where it explains its calculations, what precision a count of a given size actually supports. | **P1 – High** | _Amended 2026-08-06 (Rev N, DCR-025)._ Previously required "at minimum 2 decimal places", which contradicted SYS-232 and asserted a precision the counts do not carry: a 200-cell differential has a resolution of 0.5 percentage points, so 66.67% implies a precision three orders finer than the measurement, next to a confidence interval spanning roughly ±6 points. The digits are retained because low-frequency categories benefit from them and because a laboratory may match an existing report format — but the tool now explains what they mean rather than implying they are significant. |
...
| URS-034 | The rounding policy applied to reported percentages SHALL be selectable per configuration profile, from: **largest remainder** (default; totals exactly 100% with no category displaced by more than one unit of the last decimal shown), **largest count** (totals exactly 100% with the whole residual applied to the largest category), or **independent** (each rounded on its own; the total may deviate from 100%). The policy in force SHALL be stated with the result. | **P1 – High** | _Amended 2026-08-05 (Rev L, DCR-010)._ Rev E replaced the original "apply the residual to the largest-count category" wording with the largest-remainder method, on the grounds that the original displaces one category by up to ~2 points at integer precision. That reasoning stands and largest remainder remains the default. But rounding a differential is a **reporting policy, not a mathematical necessity**, and laboratories differ: some procedures specify the largest-count rule, and some prefer honest independent rounding even though the total then reads 99% or 101%. Mandating one contradicts the design principle that institutional variation lives in configuration. All three are therefore offered, and the report states which was used. |
...
| URS-037 | The system SHALL report the statistical uncertainty of each differential percentage arising from the finite number of cells counted, as a confidence interval whose level is configurable. A zero count SHALL be reported as a bounded interval rather than as certainty. | **P1 – High** | _New 2026-08-05 (Rev H, DCR-007)._ A differential count is a sample, and the observed percentage carries sampling error that is large at the counts used in practice and proportionally largest for the rare populations carrying the most diagnostic weight — the sampling variability quantified for manual differential counting in [S8]. An observed 20% blasts at 200 cells has a 95% interval of 15.0–26.1%, which spans the 20% AML threshold; at 500 cells it is 16.7–23.7% and still spans it. Reporting the point estimate alone presents a precision the count does not support. Stating the interval also gives ICSH §2.6's direction to extend the count near a critical threshold [S1] an operational test. |
| URS-038 | Where a configuration profile defines a diagnostic threshold, the system SHALL indicate when the confidence interval for that quantity spans the threshold — that is, when the count does not establish which side of it the true value lies on. The indication SHALL be informational and SHALL NOT block completion. | **P1 – High** | _New 2026-08-05 (Rev I, DCR-008)._ ICSH 2008 §2.6 [S1] directs that the count be extended "if the abnormal cell count is very close to a critical threshold for disease stratification or to a low threshold (e.g. 5%)". An interval straddling the threshold is the operational form of "very close". This is also the clinical need behind URS-042: the stakeholder asked for Continue Counting because "the resulting percentages are borderline", and the system can now say when they are. Non-blocking for the same reason as URS-041 — a paucicellular aspirate may make an extended count impossible, and the operator is the one who knows that. |
| URS-039 | The system SHALL support derived quantities expressed as a percentage of a subset of counted categories, in addition to ratios. | **P1 – High** | _New 2026-08-05 (Rev I, DCR-008)._ Blasts expressed as a percentage of non-erythroid cells is the erythroleukaemia rule of the WHO 4th edition. The **2016 revision** [S10] withdrew it in favour of counting blasts against all nucleated cells &mdash; WHO 2022 and ICC 2022 [S6] retained that denominator rather than introducing it &mdash; but laboratories still report it when comparing against historical results, and in a marrow with expanded erythropoiesis the two figures can fall on opposite sides of the 20% boundary. A configuration-driven system must be able to express both. Unlike a ratio, a subset percentage has a real denominator count and therefore carries a confidence interval (REF-001 §3.8). |
...
| URS-108 | The system SHALL offer an audio mode in which each counted category sounds a distinct pitch, derived from the profile's category order and not separately configurable. The mode SHALL be operator-selectable and SHALL NOT alter any counted value or visual feedback. | **P2 – Medium** | _New 2026-08-07 (DCR-036)._ The operator's eyes are on the microscope, so audio is the only feedback channel that is free — which is why sound exists in this application at all. The present click carries one bit: *a* key registered. A per-category tone carries the bit that matters, *which* key registered, making a displaced hand or a wrong key catchable without looking up. Precedent: the variable-pitch pulse oximeter, by which clinicians detect desaturation by ear. The claim is discrimination, not identification — the operator need not name a pitch, only notice that this press did not sound like the last fifty. Deriving pitch from category order rather than configuring it means no tone can disagree with the profile it belongs to. |
...
| L | 2026-08-05 | QMS | URS-034 amended again: the rounding policy is now selectable rather than mandated, with largest remainder as the documented default. Decimal precision likewise. Mandating a single reporting policy contradicted the design principle that institutional variation lives in configuration. |
...
| N | 2026-08-06 | QMS | URS-032 amended: precision is configurable, not fixed at a minimum of 2 places, resolving a direct contradiction with SYS-232. The requirement now also obliges the tool to state what precision a count of a given size supports, rather than displaying digits that imply more than the measurement carries. Independent review finding C-3. |
| M | 2026-08-06 | QMS | URS-039 rationale corrected: the non-erythroid blast denominator was withdrawn by the **WHO 2016 revision** (Arber et al., [S10]), not by WHO 2022. WHO 2022 and ICC 2022 retained the all-nucleated-cells denominator. Citation error found by independent review (DCR-017). |
| I | 2026-08-05 | QMS | URS-038 and URS-039 added: a near-threshold advisory when a confidence interval spans a configured diagnostic threshold, closing the ICSH §2.6 gap recorded in REF-001; and subset-percentage formulas, so both the current and the historical blast conventions can be expressed. |
