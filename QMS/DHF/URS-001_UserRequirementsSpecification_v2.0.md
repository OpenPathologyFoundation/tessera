...
| URS-012 | The system SHALL display cell type categories appropriate to the selected specimen type, as defined in the active configuration profile. | **P0 – Critical** | Categories vary by specimen type and institution. |
...
| URS-035 | The system SHALL compute and display derived formulas defined in the configuration profile (e.g., M:E ratio) in real time as counts change. If a formula denominator is zero, it SHALL display "N/A." | **P1 – High** | _New._ M:E ratio and other derived parameters are standard clinical outputs for bone marrow differentials. |
...
| URS-105 | The system SHALL allow configuration of the target cell count per specimen type. The default targets SHALL follow evidence-based guidelines (PB: 200 per CLSI H20-A2; BM: 500 per CAP recommendation, with literature noting 200–300 cells are clinically sufficient). | **P1 – High** | _Amended from v1.0 URS-102._ Specific defaults documented with rationale. Institutions may override. |
